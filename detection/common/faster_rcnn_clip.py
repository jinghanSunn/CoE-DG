...
class FastRCNN(nn.Module):
    def __init__(self, config, average_pool=True, final_dim=768, enable_cnn_reg_loss=False, model=None):
        """
        :param config:
        ...
        self.image_feat_precomputed = False
        if self.image_feat_precomputed:
            if config.NETWORK.IMAGE_SEMANTIC:
                self.object_embed = torch.nn.Embedding(num_embeddings=9, embedding_dim=128)
            else:
            ...
            self.output_conv5 = True
            # if self.num_layers == 18:
            #     self.model = resnet18(pretrained=True, pretrained_model_path=self.pretrained_model_path,
            #                              expose_stages=[4])
            #     block = BasicBlock
            # elif self.num_layers == 34:
            #     self.model = resnet34(pretrained=True, pretrained_model_path=self.pretrained_model_path,
            #                              expose_stages=[4])
            #     block = BasicBlock
            # elif self.num_layers == 50:
            #     self.model = resnet50(pretrained=True, pretrained_model_path=self.pretrained_model_path,
            #                              expose_stages=[4], stride_in_1x1=self.stride_in_1x1)
            #     block = Bottleneck
            # elif self.num_layers == 101:
            #     self.model = resnet101(pretrained=True, pretrained_model_path=self.pretrained_model_path,
            #                               expose_stages=[4], stride_in_1x1=self.stride_in_1x1)
            #     block = Bottleneck
            # elif self.num_layers == 152:
            #     self.model = resnet152(pretrained=True, pretrained_model_path=self.pretrained_model_path,
            #                               expose_stages=[4], stride_in_1x1=self.stride_in_1x1)
            #     block = Bottleneck
            # else:
            ...
            # self.roi_head_feature_extractor = self.model._make_layer(block=block, planes=512, blocks=3,
            #                                                             stride=2 if not self.c5_dilated else 1,
            #                                                             dilation=1 if not self.c5_dilated else 2,
            #                                                             stride_in_1x1=self.stride_in_1x1)
            self.roi_head_feature_extractor = self.model._make_layer(planes=512, blocks=3,
                                                                        stride=2 if not self.c5_dilated else 1,)
                                                                        # dilation=1 if not self.c5_dilated else 2,)
                                                                        # stride_in_1x1=self.stride_in_1x1)
            ...
            self.model.frozen_parameters(frozen_stages=frozen_stages,
                                            frozen_bn=config.NETWORK.IMAGE_FROZEN_BN)

            if self.enable_cnn_reg_loss:
                self.regularizing_predictor = torch.nn.Linear(2048, 9)
    ...
    def forward(self, images, boxes, box_mask, im_info, classes=None, segms=None, mvrc_ops=None, mask_visual_embed=None):
        ...
        """
        # print(im_info) # [[640, 640, 0.2459546925566343], [640, 640, 0.23946435604568728],...]
        # im_info = (im_info)
        ...
        if self.image_feat_precomputed:
            ...
            img_feats = self.model(images)
            # print(img_feats['body4'].shape) # [b, 2048, 23, 20]
            rois = torch.cat((
                box_inds[:, 0, None].type(img_feats['body4'].dtype),
                boxes[box_inds[:, 0], box_inds[:, 1]],
            ), 1)
            roi_align_res = self.roi_align(img_feats['body4'], rois).type(img_feats['body4'].dtype)

            if segms is not None:
                pool_layers = self.head[1:]
                post_roialign = self.roi_head_feature_extractor(roi_align_res)
                post_roialign = post_roialign * segms[box_inds[:, 0], None, box_inds[:, 1]].to(dtype=img_feats['body4'].dtype)
        ...
        final_feats = self.obj_downsample(feats_to_downsample.to(dtype=img_feats['body4'].dtype))
