...
class Transformer(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        encode_x = self.encode(src, src_mask) # [16, 50, 512]
        # return self.decode(encode_x, src_mask, tgt, tgt_mask)
        return encode_x[:, 0], self.decode(encode_x[:, 1:], src_mask[:,:, 1:], tgt, tgt_mask)
        # return encode_x[:, 0], self.decode(encode_x[:, :1], src_mask[:,:, :1], tgt, tgt_mask)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, hidden_states, src_mask, tgt_mask, memory):
        # print(x.shape) # torch.Size([16, 91, 512]) 
        # print(hidden_states.shape) # torch.Size([16, 49, 512]) 
        # print(src_mask.shape) # torch.Size([16, 1, 49])  
        # print(tgt_mask.shape) # torch.Size([16, 91, 91]) 
        # print(memory.shape) # torch.Size([16, 91, 1536]) 
        m = hidden_states
...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        self.rm_d_model = args.rm_d_model # 512

        tgt_vocab = self.vocab_size + 1
        # print(tgt_vocab)
        self.cls_token = nn.Parameter(torch.randn(1, 1, args.d_model))
        self.model = self.make_model(tgt_vocab)
        # self.loc_embed = nn.Embedding(101,512)
        self.loc_embed = nn.Embedding(101,2048)
        self.text_embed = nn.Embedding(tgt_vocab,2048)
        self.logit = nn.Linear(args.d_model, tgt_vocab)
        # self.classifier = nn.Linear(4*5*2048, 5*2048)
        self.lin = nn.Linear(args.d_model, args.num_classes)
        # self.avg = nn.AvgPool2d(4*5, 5)

    def init_hidden(self, bsz):
        return []

    def _prepare_feature(self, fc_feats, att_feats, att_masks, location_rep=None, label_name=None):
        if location_rep is not None:
            if self.args.loc_mode=='convert':
                # location_rep = self.loc_embed((location_rep)).reshape((att_feats.shape[0], -1)).unsqueeze(1)
                location_rep = self.loc_embed(location_rep).squeeze() # [b, 4*N^b, 2048] 
                # b, n, c = location_rep.shape
                # location_rep = self.lin(location_rep.reshape((b, -1))).reshape(b,5,c)
                # location_rep = self.avg(location_rep)
            if label_name is not None:
                label_name = self.text_embed(label_name)
                att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(torch.cat([att_feats, location_rep, label_name], dim=1), att_masks) # mode2
            else:
                # att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats+location_rep, att_masks) # mode1
                att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(torch.cat([att_feats, location_rep], dim=1), att_masks) # mode2
        else:
        ...
        cls_token = memory[:, 0]
        pred = cls_token
        return fc_feats[..., :1], att_feats[...,1:, :], memory[:, 1:], att_masks[:, :, 1:], pred, cls_token
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None):
        ...
        if att_masks is None:
            b, n = att_feats.shape[:2]
            att_masks = att_feats.new_ones((b,n+1), dtype=torch.long)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None, location_rep=None, label_name=None):

        if location_rep is not None:
            if self.args.loc_mode=='convert':
                location_rep = self.loc_embed(location_rep).squeeze() 
            if label_name is not None:
                label_name = self.text_embed(label_name) 
                att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(torch.cat([att_feats, location_rep, label_name], dim=1), att_masks, seq)
            else:
                att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(torch.cat([att_feats, location_rep], dim=1), att_masks, seq) # mode2
        else:
            att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)
        b, n, _ = att_feats.shape
        cls_tokens = repeat(self.cls_token, '() n d -> b n d', b = b)
        att_feats = torch.cat((cls_tokens, att_feats), dim=1)
        cls_token, out = self.model(att_feats, seq, att_masks, seq_mask) 
        pred = cls_token
        outputs = F.log_softmax(self.logit(out), dim=-1)
        return pred, outputs, cls_token
