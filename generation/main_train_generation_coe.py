"""
Main training script for report generation with detection-based CoE (Cooperation of Experts).

This script trains the R2Gen model with object detection guidance for medical report generation.
"""
import argparse
import warnings
...
def main():
    ...
    if args.dataset_name in ['mimic_detection_pre', 'mimic_detection_multi']:
        # Detect abnormality using pretrained detection model before training
        generator_model = R2GenModel(args, tokenizer, visual='pre_object')
    else:
        generator_model = R2GenModel(args, tokenizer, visual='object')

    retinanet_model = model.resnet101(num_classes=9, pretrained=True)
    retinanet_model.load_state_dict(torch.load(args.resume_detection).module.state_dict(), strict=True)

    # Freeze detection model parameters
    for param in retinanet_model.parameters():
        param.detach_()
    ...
    optimizer = build_optimizer(args, generator_model)
    lr_scheduler = build_lr_scheduler(args, optimizer)

    # Build trainer and start training
    if args.dataset_name == 'mimic_detection_pre':
        # Use predetected abnormality
        trainer = TrainerPre(
            generator_model, retinanet_model, criterion, metrics, optimizer,
            args, lr_scheduler, train_dataloader, test_dataloader
        )
